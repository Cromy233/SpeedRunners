execute as @a[gamemode=!spectator] if predicate speedrunner:0.8chance at @s run summon marker ~ ~ ~ {Tags:["SR_Disaster_Heavyrain"]}
execute as @a[gamemode=!spectator] if predicate speedrunner:0.8chance at @s run summon marker ~ ~ ~ {Tags:["SR_Disaster_Heavyrain"]}
execute at @a[gamemode=!spectator] run spreadplayers ~ ~ 32 64 false @e[tag=SR_Disaster_Heavyrain,sort=nearest,limit=2]
