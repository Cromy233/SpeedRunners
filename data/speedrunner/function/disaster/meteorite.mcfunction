execute as @a[gamemode=!spectator] if predicate speedrunner:0.8chance at @s run summon marker ~ ~ ~ {Tags:["SR_Disaster_Meteorite"]}
execute at @a[gamemode=!spectator] run spreadplayers ~ ~ 32 64 false @n[tag=SR_Disaster_Meteorite]
